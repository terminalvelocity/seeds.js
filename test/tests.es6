...
describe('Seeds', function() {
  ...
  it('should have a default configuration', function() {
    cli = new Seeds();
    expect(cli.config.apps.sails[0].name).to.equal('api');
    expect(cli.config.apps.sails[0].port).to.equal(1776);
    expect(cli.config.apps.ember[0].name).to.equal('frontend');
    expect(cli.config.apps.ember[0].port).to.equal(4200);
...
describe('Aliases', function() {
  ...
  it('should return help if command passed in doesnt match a command', function() {
    let noMatchArgs = Seeds.cli(['doesnotexist']).args;
