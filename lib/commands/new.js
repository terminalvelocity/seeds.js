...
module.exports = function(cli) {
  ...
  function generateAppFiles() {
    return cli.exec('npm install').then(function() {
      runExternalCommand(`./node_modules/.bin/sails`, ['new', 'api']).then(setupSails);
      runExternalCommand(`./node_modules/.bin/ember`, ['new', 'frontend', '--skip-git']).then(setupEmber);
    });
  }
  ...
  function setupSails() {
    cleanUpSails().then(function() {
      ...
      cli.copyTemplates(`${cli.basepath()}/lib/templates/api`, `${cli.cwd()}/${appName}/api`, {clobber: true}, function(err) {
        if (err) { return cli.debug(err); }
  ...
  function setupEmber() {
    ...
    return runExternalCommand(`${cli.cwd()}/${appName}/frontend/node_modules/.bin/ember`, emberPackages, { cwd: `${cli.cwd()}/${appName}/frontend` }).then(function() {
      cli.copyTemplates(`${cli.basepath()}/lib/templates/frontend`, `${cli.cwd()}/${appName}/frontend`, {clobber: true}, function(err) {
        if (err) { return cli.debug(err); }
